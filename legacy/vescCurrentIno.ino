 ...
 *                               PICTRL,esp32_ms,i_target_a,i_meas_a,i_cmd_a,omega_rps,actual_hz,target_lb,i_error_a,i_ff_a,i_int_a,i_cmd_unsat_a
...
// Runtime toggles for zero-crossing reset features (can be changed via PI_RESET_* commands)
static bool g_piResetIntOnCrossing = true;   // reset integrator at zero crossing
static bool g_piResetFFOnCrossing = true;    // reset feedforward at zero crossing
...
static void pollPiLoop(uint32_t nowMs, uint32_t nowUs) {
  ...
  // Detect target current sign change BEFORE running controller to catch zero crossing early
  // This prevents integrator and feedforward from accumulating in the wrong regime
  bool target_crossed_zero = (g_piTargetAmpsPrev >= 0.0f && g_piTargetAmps < 0.0f) || 
                             (g_piTargetAmpsPrev <= 0.0f && g_piTargetAmps > 0.0f);
  if (target_crossed_zero) {
    if (g_piResetIntOnCrossing) {
      g_piIntegrator = 0.0f;
    }
    if (g_piResetFFOnCrossing) {
      g_piFF = 0.0f;  // Will be overwritten by piCurrentController, but we set it preemptively
    }
  }
  ...
  if (++g_piEmitDecimCnt >= g_piEmitEveryN) {
    ...
    sendHostFmt("PICTRL,%lu,%.4f,%.4f,%.4f,%.3f,%.2f,%.3f,%.4f,%.4f,%.4f,%.4f",
        (unsigned long)nowMs,
        g_piTargetAmps, i_meas, i_cmd, omega,
        actual_hz, g_piTargetLbs,
        g_piError, g_piFF, g_piIntegrator, g_piICmdUnsat);
  }
}

// PICFG snapshot reply (for PI_CONFIG).
static void sendPiConfigLine() {
  sendHostFmt("PICFG,%.4f,%.4f,%.4f,%.6f,%.4f,%.4f,%.3f,%.4f,%.4f,%d,%lu,%d,%.3f,%.4f,%d,%d",
...
static void processCommand(const String& cmd) {
  ...
  if (cmd.startsWith("PI_RESET_FF,")) {
    bool on = parseFloat(cmd.substring(12)) != 0.0f;
    g_piResetFFOnCrossing = on;
    sendHostFmt("OK,PI_RESET_FF,%d", on ? 1 : 0);
    return;
  }
