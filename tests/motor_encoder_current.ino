 ...
 *   pwm: 0-100%
 * Current control: CURRENT,<seconds>,<current_A>,<kp>,<ki>,<kd>,<F|B> (current_A may be negative)
 *
...
// Sampling (raised conservatively for faster current control while staying robust)
static constexpr uint32_t SAMPLE_HZ           = 250;
...
// +1.0f: raw shunt/INA polarity matches “positive setpoint → positive reported A”. Use -1.0f if you command +1 A but measured current sits near −1 A (shunt leg / INA inputs swapped vs convention).
#define CURRENT_SENSE_SIGN (-1.0f)
#define CURRENT_SAMPLES 4
static constexpr int      SUPPLY_SAMPLES                 = 2;
static constexpr uint32_t SUPPLY_REFRESH_INTERVAL_US     = 20000UL;  // 50 Hz reference tracking
...
static float g_zeroReferenceMV = 0.0f;
static uint32_t g_lastSupplyRefreshUs = 0;
...
static float readSupplyZeroReferenceMV() {
    analogRead(SUPPLY_SENSOR_PIN);  // settle ADC mux after switching channels

    uint32_t sum = 0;
    for (int i = 0; i < SUPPLY_SAMPLES; i++) {
        sum += analogRead(SUPPLY_SENSOR_PIN);
    }

    float supplyV = ((sum / (float)SUPPLY_SAMPLES) / ADC_MAX) * VREF * DIVIDER_RATIO;
    return (supplyV * 1000.0f) / 2.0f;
}

static void refreshCurrentZeroReference(uint32_t nowUs, bool force = false) {
    if (overrideZeroMV >= 0.0f) {
        g_zeroReferenceMV = overrideZeroMV * CURRENT_SENSOR_OUTPUT_DIVIDER;
        g_lastSupplyRefreshUs = nowUs;
        return;
    }

    if (!force && (uint32_t)(nowUs - g_lastSupplyRefreshUs) < SUPPLY_REFRESH_INTERVAL_US) {
        return;
    }

    g_zeroReferenceMV = readSupplyZeroReferenceMV();
    g_lastSupplyRefreshUs = nowUs;
}

static float readCurrentAmps(uint32_t nowUs) {
    refreshCurrentZeroReference(nowUs);

    analogRead(CURRENT_SENSOR_PIN);  // settle ADC mux after switching channels
    ...
    return CURRENT_SENSE_SIGN * (sensorMV - g_zeroReferenceMV) / SENSITIVITY_MV_PER_A;
}
...
static void checkZeroOverride() {
    ...
    if (btn && !lastButtonState) {
        ...
        g_zeroReferenceMV = overrideZeroMV * CURRENT_SENSOR_OUTPUT_DIVIDER;
        g_lastSupplyRefreshUs = micros();
        if (g_state == State::IDLE || g_state == State::ARMED) {
            Serial.printf("ZERO,%.1f\n", overrideZeroMV);
        }
    }
    lastButtonState = btn;
}
...
static void processCommand(const String& cmd) {
    if (cmd.startsWith("CURRENT,")) {
        ...
        // Format: CURRENT,<seconds>,<current_A>,<kp>,<ki>,<kd>,<F|B>  (current_A signed: + or -)
    ...
    if (cmd == "GO") {
        ...
        g_lastCmdPwm = 0;
        g_lastDirSign = 0;
        refreshCurrentZeroReference(g_drillStartUs, true);
...
static bool processData() {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        timeS[i] = (float)(g_rawSamples[i].timestamp_us - g_rawSamples[0].timestamp_us) / 1e6f;
    }
    ...
    vel[0] = (dt > 0.0f) ? (pos[1] - pos[0]) / dt : 0.0f;
    dt = timeS[N - 1] - timeS[N - 2];
    vel[N - 1] = (dt > 0.0f) ? (pos[N - 1] - pos[N - 2]) / dt : 0.0f;
    for (size_t i = 1; i < N - 1; i++) {
        dt = timeS[i + 1] - timeS[i - 1];
        vel[i] = (dt > 0.0f) ? (pos[i + 1] - pos[i - 1]) / dt : 0.0f;
    }

    std::vector<float> acc(N);
    dt = timeS[1] - timeS[0];
    acc[0] = (dt > 0.0f) ? (vel[1] - vel[0]) / dt : 0.0f;
    dt = timeS[N - 1] - timeS[N - 2];
    acc[N - 1] = (dt > 0.0f) ? (vel[N - 1] - vel[N - 2]) / dt : 0.0f;
    for (size_t i = 1; i < N - 1; i++) {
        dt = timeS[i + 1] - timeS[i - 1];
        acc[i] = (dt > 0.0f) ? (vel[i + 1] - vel[i - 1]) / dt : 0.0f;
    }

    g_processed.resize(N);
    for (size_t i = 0; i < N; i++) {
        g_processed[i].time_s       = timeS[i];
...
void setup() {
    ...
    refreshCurrentZeroReference(micros(), true);
    ...
    switch (g_state) {
        ...
        case State::RUNNING: {
            ...
            if ((int32_t)(nowUs - g_nextSampleUs) >= 0) {
                ...
                s.current_A    = readCurrentAmps(nowUs);
                s.error_A      = 0.0f;
        ...
        case State::CURRENT_CONTROL: {
            ...
            if ((int32_t)(nowUs - g_nextSampleUs) >= 0) {
                ...
                s.timestamp_us = nowUs;
                s.count        = readEncoderCount();
                float current = readCurrentAmps(nowUs);
